...
tumblr.fn._request = function(obj, fn) {
  ...
  if (!/^\/(user\/posts|blog\/(info|avatar))/.test(settings.path)) {
    if (this.key && this.secret && this._oauth_access) {
      ...
      cached._req = req = this._oa[obj.method.toLowerCase()](url.format(settings), this.access_key, this.access_secret).on('response', onRes);
    }
    else {
...
tumblr.fn.follow = function(url, fn) {
  return this._post({
    type : 'blog',
    path : '/follow',
    query : {
